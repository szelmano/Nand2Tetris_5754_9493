...
function BuildFullFunctionName(functionName : String, currentInputFile : File) : String {
  if(functionName.indexOf("$") >= 0 or functionName.indexOf(".") >= 0) {
    return functionName
  }
...
function ProcessVmFile(currentInputFile : File, outputWriter : BufferedWriter) {
  ...
  try {
    while(scanner.hasNextLine()) {
      ...
      WriteFunction(parts[1], parts[2], currentInputFile, outputWriter)

    } else if(parts[0] == "call") {
      WriteCall(parts[1], parts[2], currentInputFile, outputWriter)
...
function WriteFunction(functionName : String, numLocalsText : String, currentInputFile : File, outputWriter : BufferedWriter) {
  var fullFunctionName = BuildFullFunctionName(functionName, currentInputFile)

  currentFunctionName = fullFunctionName
  var numLocals = Integer.parseInt(numLocalsText)

  W(outputWriter, "// function " + functionName + " " + numLocalsText)
  W(outputWriter, "(" + fullFunctionName + ")")
...
function WriteCall(functionName : String, numArgsText : String, currentInputFile : File, outputWriter : BufferedWriter) {
  var fullFunctionName = BuildFullFunctionName(functionName, currentInputFile)
  var returnLabel = "RETURN_" + fullFunctionName + "_" + callCounter
...
function WriteInit(outputWriter : BufferedWriter) {
  W(outputWriter, "// bootstrap")
  W(outputWriter, "@256")
  W(outputWriter, "D=A")
  W(outputWriter, "@SP")
  W(outputWriter, "M=D")

  var bootstrapFile = new File("Sys.vm")
  WriteCall("Sys.init", "0", bootstrapFile, outputWriter)
}
